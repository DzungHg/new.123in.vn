...
class FieldsetGroupConfig extends WireData {
	...
	public function getConfigInputfields(Field $field, InputfieldWrapper $inputfields) {
		...
		$f->description = 
			$this->_('The “tab” option applies to non-nested usage in page editor.') . ' ' . 
			$this->_('The “borderless fieldset” is an invisible fieldset with no border, header, label, description or visibility configuration—only the fields within are shown.'); 
		$f->addOption(0, $this->_('Fieldset'));
		$f->addOption(-1, $this->_('Borderless fieldset'));
