...
class FormBuilderMaker extends Wire {
	...
	public function makeInputfieldForm(FormBuilderForm $fbForm, $requestPageNum = 0) {
		return $this->arrayToInputfields($fbForm->getArray(), $requestPageNum);
	}
	...
	public function arrayToInputfields(array $a, $requestPageNum = 0, $currentPageNum = 1, $inputfields = null, $submitType = null) {
		...
		$numFields = 0;
		$isAdmin = $this->wire('page')->process === 'ProcessFormBuilder'; // are we rendering this for the admin?
		$notes = '';
		$cnt = 0;
		...
		foreach($items as $name => $data) {
			...
			if($inputType === 'FormBuilderPageBreak' && $isForm && !$isAdmin && $requestPageNum > 0) { 
